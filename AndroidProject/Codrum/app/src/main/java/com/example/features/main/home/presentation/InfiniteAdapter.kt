...
const val NUM_BANNERS = 3

class InfiniteAdapter(var list: MutableList<Int>) :
    ...
    override fun onBindViewHolder(holder: InfiniteViewHolder, position: Int) {
        holder.onBind(list[position % NUM_BANNERS]) // position에 실제 배너의 개수를 나눈 나머지 값을 사용한다. (여기서는 3개라 하드코딩으로 3을 넣음)
    }
