...
img_color = cv2.imread('./img/original.jpg', cv2.IMREAD_COLOR)
img_original = cv2.cvtColor(img_color, cv2.COLOR_BGR2GRAY)

img_template = cv2.imread('./img/image.jpg', cv2.IMREAD_GRAYSCALE)
...
for original_x in range(original_w - template_w):
    for original_y in range(original_h - template_h):
        ...
        SAD = np.abs(np.subtract(original_pixel, template_pixel, dtype=float))
        SAD = SAD.sum()
